...
class GeoblockingIcon extends HTMLElement {
  ...
  private dataLabel: HTMLSpanElement;
  ...
  attributeChangedCallback(
    ...
    if (name === 'area') {
      return this.updateArea(newValue as AreaType);
    }
    console.warn(`Invalid attribute name: ${name}`);
  }

  private updateArea(area: AreaType) {
    if (!AREAS.includes(area as AreaType)) {
...
const AREA_CONFIG: Record<AreaType, { label: string }> = {
  ALL: {
    label: 'Worldwide',
  },
  EUR_PLUS: {
