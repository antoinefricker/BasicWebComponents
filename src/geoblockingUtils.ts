...
] as const satisfies readonly GeoblockingAreaUnsafe[];
...
export const getGeoblockingAreaById = (id: string | null): GeoblockingArea | null => {
  ...
  const geoblockingArea = GEOBLOCKING_AREAS.find((area) => area.id === id);
  return geoblockingArea || null;
};

type GeoblockingAreaUnsafe = {
  id: string;
  fullLabel: string;
  shortLabel: string;
};

export type GeoblockingArea = Omit<GeoblockingAreaUnsafe, 'id'> & {
